// Classic app (cute theme), FINAL_THRESHOLD=280
const FINAL_THRESHOLD = 280;

// Self-contained app.js (no fetch).
// Prices are embedded below:
...
function homeView() {
  ...
  view.innerHTML = `
    <div class="panel center">
      ...
      ${!canFinal ? `<p style="margin-top:10px; color:#fca5a5;">U1 needs at least 280 to enter final challenge.</p>` : ""}
    </div>
  `;
  $("#rollBtn").onclick = onRoll;
  $("#finalBtn").onclick = () => {
    if (state.balance < FINAL_THRESHOLD) {
      alert("U1 needs at least 280 to enter final challenge");
      return;
    }
    finalChallengeIntro();
  };
...
function findRangeForNumber(num) {
  return state.ranges.find(r => num >= r.start && num <= r.end);
}

function startPurchase(num) {
  const range = findRangeForNumber(num);
  if (!range) {
    alert("No price range found for this box number.");
    homeView();
    return;
  }
  ...
  state.session = {
    ...
    purchasePrice: range.purchasePrice,
    items: range.items,
    guess: null
  };
...
function renderPrediction() {
  ...
  const options = s.items.map(({name}) => {
    return `<button class="choiceBtn" data-name="${name}">${name}</button>`;
  }).join("");
  ...
  view.innerHTML = `
    <div class="panel">
      ...
      <p class="kicker">Choose your prediction (the possible items in this series):</p>
  ...
  document.querySelectorAll(".choiceBtn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const guess = e.currentTarget.getAttribute("data-name");
      state.session.guess = guess;
      renderOpenPrompt();
    });
  });
...
function renderOpenPrompt() {
  ...
  document.querySelectorAll(".choiceBtn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const resultName = e.currentTarget.getAttribute("data-name");
      completePurchase(resultName);
    });
  });
  $("#abort").onclick = () => {
    if (s._deducted) state.balance += s.purchasePrice;
    state.session = null;
    updateHUD();
    homeView();
  };
...
function completePurchase(resultName) {
  ...
  view.innerHTML = `
    <div class="panel center">
      ...
      <p class="kicker">
        Item value: $${itemPrice}${s.guess === resultName ? " • Correct guess bonus: +$10 🎉" : ""}
      </p>
...
function finalYes() {
  view.innerHTML = `
    <div class="panel center">
      ...
      <input id="answer" type="text" placeholder="请输入答案…" style="padding:10px;border-radius:8px;border:1px solid rgba(148,163,184,0.35);background:#0b1020;color:white;width:280px;"/>
      <div class="btnrow" style="justify-content:center; margin-top: 12px;">
